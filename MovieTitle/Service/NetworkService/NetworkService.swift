...
protocol RemoteServiceProtocol {
    func fetch<T>(pathUrl: String, type: T.Type, page: Int, completionHandler completion: @escaping (Result<T, ErrorModel>) -> Void) where T: Codable
}

class RemoteService: RemoteServiceProtocol {
    
    func fetch<T>(pathUrl: String, type: T.Type, page: Int = 1, completionHandler completion: @escaping (Result<T, ErrorModel>) -> Void) where T: Codable{
